...
function sortTodolist(viewTitle, viewDetail){
  ...
  let minuteStartHourCurrent = parseInt(viewTitle.querySelector(".start-time").innerText.split(":")[1]);
    // sort
    let lengthChild = todoListTable.childNodes;
    for(let i = lengthChild.length - 1; i >= 0; i--) {
      if((i % 2) == 0) {
        let hourStartHourItem = parseInt(lengthChild[i].querySelector(".start-time").innerText.split(":")[0]);
        let minuteStartHourItem = parseInt(lengthChild[i].querySelector(".start-time").innerText.split(":")[1]);
        ...
        || (hourStartHourItem === hourStartHourCurrent && minuteStartHourItem > minuteStartHourCurrent)){
          todoListTable.insertBefore(viewDetail, lengthChild[i]);
          todoListTable.insertBefore(viewTitle, viewDetail);
        }
      } else {
        todoListTable.insertAfter(viewDetail, lengthChild[i]);
        todoListTable.insertAfter(viewTitle, viewDetail);
      }
    }
}
