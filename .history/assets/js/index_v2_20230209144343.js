...
function sortTodolist(viewTitle, viewDetail){
  ...
  let minuteStartHourCurrent = parseInt(viewTitle.querySelector(".start-time").innerText.split(":")[1]);
    // sort
    let itemChild = todoListTable.querySelectorAll(".view-title");
    for(let i = itemChild.length - 1; i >= 0; i--) {
        let hourStartHourItem = parseInt(itemChild[i].querySelector(".start-time").innerText.split(":")[0]);
        let minuteStartHourItem = parseInt(itemChild[i].querySelector(".start-time").innerText.split(":")[1]);
        ...
        || (hourStartHourItem === hourStartHourCurrent && minuteStartHourItem > minuteStartHourCurrent)){
          todoListTable.insertBefore(viewDetail, itemChild[i]);
...
function checkOut() {
  let todoListTable = document.getElementById("view-todolist");
  let statusArray = [];
  todoListTable.querySelectorAll(".status-work-value").forEach((item) => {
    statusArray.push(item.innerHTML.trim());
  });

  let check = statusArray.every((item) => {
    return (item.innerHTML.trim() === "Todo" || item.innerHTML.trim() === "Doing")
  });
  // if(item.innerHTML.trim() === "Todo" || item.innerHTML.trim() === "Doing") {
  //   alert("Không thể check-out khi có công việc đang ở trạng thái Todo, Doing!");
  //   check = false;
  //   return;
  // }

  if(!check) {
  todoListTable.querySelectorAll(".add-work").forEach((item) => {
    if(item.querySelector(".name-work-form").value.trim() != "") {
      create(item.getAttribute("id").split("-")[2]);
    };
  });
  todoListTable.childNodes.forEach((item, index) => {
    if(index % 2 != 0){
      if(item.querySelector(".detail-display").innerText === "" && item.querySelector(".result-display").innerText === "") {
        item.style.display = "none";
        item.previousSibling.querySelectorAll("td").forEach((item) => {item.style.borderBottom = "1px solid #dbdbdb"});
     } else {
        item.querySelector(".control").remove();
     }
      }
    });
  }
