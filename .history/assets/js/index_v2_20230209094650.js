...
function create(idElement) {
  ...
  todoListTable.childNodes.forEach((item, index) => {
    if((index % 2) == 0) {
      let hourStartHourItem = item.querySelector(".start-time").innerText.split(":")[0];
      let minuteStartHourItem = item.querySelector(".start-time").innerText.split(":")[1];
      ...
      || (hourStartHourItem === hourStartHourCurrent && minuteStartHourItem > minuteStartHourCurrent)){
        todoListTable.replace(viewTitle, item);
        todoListTable.replace(formDetail, item.nextSibling);
        todoListTable.replace(formDetail, item.nextSibling);
        todoListTable.replace(formDetail, item.nextSibling);
      }
    }
