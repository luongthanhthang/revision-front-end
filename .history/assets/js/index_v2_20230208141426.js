...
import addFormDetail from "./function/todolist/addFormDetail.js";
  ...
  function onloadLocalStorageTodoList() {
    ...
    Object.values(localStorage).forEach((item) => {
      ...
      viewDetail.querySelectorAll(".showFormEdit").forEach((itemChild) => {
        itemChild.addEventListener("click", () => {
          showFormEdit(itemChild.id);
        })
      })
      viewDetail.querySelectorAll(".deleteTodoList").forEach((itemChild) => {
        itemChild.addEventListener("click", () => {
          deleteTodoList(itemChild.id);
        })
      })
  ...
  function deleteTodoList(idElement) {
    ...
    if (work === null) {
      let titleElementForm = document.getElementById(`form-title-${idElement}`);
      let detailElementForm = document.getElementById(
        `form-detail-${idElement}`
      );
  ...
  function update(elementEdit, index) {
    ...
    let nameWorkForm =
      elementEditAddWork.querySelector(".name-work-form").value;
